...
void SurfactantNarrowBandStblP1CL::InitStep1 (double new_t)
{
    ...
    */
    ...
    if(std::fabs(new_t/dt_-1)<0.0000001)//(new_t/dt_==1)//used in InitStep 3.
    {
        ...
        rhsext1.Data=0.1*rhsext.Data;// Since we choose a smaller time step dt*0.1//(std::sqrt(dt_))*
...
void SurfactantNarrowBandStblP1CL::DoStep0PatternFM (double new_t)//for pattern formulation
{
    ...
    /**< get rhs1_ = Mass*(\vec u_h^n*1/dt) , rhsw1_ is similar , which will be added to load term*/
    //{
    // ScopeTimerCL timer( "SurfactantNarrowBandStblP1CL::InitStep");
    std::cout << "SurfactantNarrowBandStblP1CL::InitStep:\n";
    ic.t= new_t;
    ...
    std::cout << " full NumUnknowns: " << full_idx.NumUnknowns() << std::endl;
    DROPS::VecDescCL rhsext( &full_idx);
    DROPS::VecDescCL rhsextw( &full_idx);
    /*
       oldls_.RowIdx= lset_vd_.RowIdx;
       oldls_.Data.resize( lset_vd_.Data.size());
       oldls_.Data= lset_vd_.Data;
    ...
    */
    ...
    //   Restrict( MG_, rhsext, rhs1);
    MassMultiply(MG_,rhsext,rhs1,lset_vd_, lsetbnd_);
    MassMultiply(MG_,rhsextw,rhsw1,lset_vd_, lsetbnd_);
    //    MassMultiply(MG_,rhsext,rhs1,lset_vd_, lsetbnd_, *v_,Bnd_v_, dt_);
    //   MassMultiply(MG_,rhsext,rhs1,lset_vd_, lsetbnd_, *v_,Bnd_v_,normal_,ic.t, dt_);//characteristic method
    //  MassMultiply1(MG_,rhsext,rhs1,lset_vd_, lsetbnd_, *v_,Bnd_v_, dt_);
    //   WriteToFile( rhs1.Data, "RHS1.txt", "RHS1");
    rhs1_.resize( rhs1.Data.size());
    rhsw1_.resize( rhsw1.Data.size());
    rhs1_= rhs1.Data;
    rhsw1_= rhsw1.Data;
...
//BDF2 method
void SurfactantNarrowBandStblP1CL::DoStep (double new_t)//do step
{
...
/////////////////////AllenCahnBaseCL////////////////////////////////
void AllenCahnP1BaseCL::SetInitialValue (instat_scalar_fun_ptr icc, double t)
{
    P1Init ( icc, ic, MG_, t);
}

void AllenCahnP1BaseCL::SetRhs (instat_scalar_fun_ptr rhs3)
{
    rhs_fun3_= rhs3;
}

void AllenCahnP1BaseCL::InitTimeStep ()
{
    iface_old.SetIdx( &oldidx_c_);
...
/// ====  AllenCahnNarrowBandStblP1CL ==== ///////////////
void AllenCahnNarrowBandStblP1CL:: Update()
{
    ScopeTimerCL timer( "AllenCahnNarrowBandStblP1CL::Update");
    std::cout << "AllenCahnNarrowBandStblP1CL::Update:\n";
    ...
    {
        ScopeTimerCL timer( "AllenCahnNarrowBandStblP1CL::setup-Matrix");
        accumulate( accus, MG_, cidx->TriangLevel(), cidx->GetBndInfo());
    }
    ...
    //     WriteToFile( Laplace.Data, "chartranspA.txt", "Laplace-Beltrami");
    //     WriteToFile( Volume_stab.Data, "Stabilization.txt", "Stab-matrix");
    //        WriteToFile( Massd.Data,"chartranspMd.txt","mass-div");
    //         WriteToFile( vd_load.Data,"chartranspload.txt","load");
    //     WriteToFile( Conv.Data,"convection.txt","convection");

    // std::cout << "AllenCahnNarrowBandStblP1CL::Update: Finished\n";
    //MatrixCL Precond3, Precond4;
    /*std::valarray<double> ones( 1., Mass.Data.num_rows());
    MatrixCL Ident(ones);*/
    /*  Precond3.LinComb(1.0, Mass.Data, sigma_*dt_, Laplace.Data, dt_*rho_, Volume_stab.Data);
...
// Implement Implicit Euler method
void AllenCahnNarrowBandStblP1CL::InitStep1 (VectorCL& rhs3, double new_t)
{
    // ScopeTimerCL timer( "AllenCahnNarrowBandStblP1CL::InitStep");
    std::cout << "AllenCahnNarrowBandStblP1CL::InitStep:\n";
    ...
    accumulate( accus, MG_, idx_c.TriangLevel(), idx_c.GetBndInfo());
    //        rhs3 = VectorCL( (1/dt_+S_/epsilon_)*vd_timeder.Data + vd_load3.Data-1/epsilon_*vd_well.Data);//Only for Backward Euler so far!!
    //original eps L+eps^{-1}f   --many exact solution is set in this way.
    rhs3 = VectorCL( (1/dt_+S_/(epsilon_*epsilon_))*vd_timeder.Data + vd_load3.Data-1/(epsilon_*epsilon_)*vd_well.Data);//Only for Backward Euler so far!!
    // L+eps^{-2}f
    /*
              WriteToFile( vd_oldic.Data, "vd_oldic.txt", "vd_oldic");
              WriteToFile( rhsext1.Data, "rhsext1.txt", "rhsext1");
...
// Implement BDF2 method
//In the first step, we use the backward Euler method
void AllenCahnNarrowBandStblP1CL::InitStep2 (double new_t)
{
    /*
    // ScopeTimerCL timer( "SurfactantcGP1CL::InitStep");
    std::cout << "SurfactantNarrowBandStblP1CL::InitStep:\n";
...
// Implement BDF2 method
// In the first step, we solve the problem with much smaller time step
void AllenCahnNarrowBandStblP1CL::InitStep3 (double new_t)
{
    /*
    // ScopeTimerCL timer( "SurfactantNarrowBandStblP1CL::InitStep");
    std::cout << "SurfactantNarrowBandStblP1CL::InitStep:\n";
...
//Implicit Euler method
void AllenCahnNarrowBandStblP1CL::DoStep1 (VectorCL& rhs3)
{
    Update();

    double c=1.0;//BDF1

    //    A_.LinComb(epsilon_,  Laplace.Data, (c/dt_+S_/epsilon_),Mass.Data, rho_*epsilon_,Volume_stab.Data,1., Massd.Data, 1, Conv.Data);
    A_.LinComb(1.0, Laplace.Data, (c/dt_+S_/(epsilon_*epsilon_)),Mass.Data, rho_,Volume_stab.Data,0., Massd.Data, 1.0, Conv.Data);
    //   WriteToFile( A_, "Matrix.txt", "system");
    //  WriteToFile( Conv.Data,"convection.txt","convection");

    std::cout << "      Before solve: res3 = " << norm(A_ * ic.Data- rhs3) << std::endl;
    {
        ScopeTimerCL timer("AllenCahnNarrowBandStblP1CL::DoStep: Solve");
        ...
        std::cout << "      After  solve: res3 = " << norm(A_ * ic.Data- rhs3) << std::endl;
    }
    std::cout << "AllenCahnNarrowBandStblP1CL::DoStep: res = " << gm_.GetResid() << ", iter = " << gm_.GetIter() << std::endl;
}

//BDF2 method
void AllenCahnNarrowBandStblP1CL::DoStep2 (VectorCL& rhs3)
{
    /*
            ic.SetIdx( &idx);
...
void AllenCahnNarrowBandStblP1CL::CommitStep ()
{
    full_idx.DeleteNumbering( MG_);
}
...
//only back forward Euler
void AllenCahnNarrowBandStblP1CL::DoStep0 (double new_t)
{
    ScopeTimerCL timer( "AllenCahnNarrowBandStblP1CL::::DoStep0");
    VectorCL rhs3;
    InitStep1( rhs3, new_t);
    DoStep1(rhs3 );/**/
    CommitStep();
    //   WriteToFile( temp_ic.Data, "Extended.txt", "mass");
...
//BDF2 method
void AllenCahnNarrowBandStblP1CL::DoStep (double new_t)
{
    /*ScopeTimerCL timer( "SurfactantExtensionP1CL::::DoStep");
     InitStep2( new_t);
     if(new_t/(new_t-oldt_)==1)
