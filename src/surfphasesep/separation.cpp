...
double Test7_chiSol (const DROPS::Point3DCL& p, double t) {
    ...
    double T=0.25;
    DROPS::Point3DCL x= (p - PosDrop - t*constant_wind(p, t));//--constant wind
    return ((0.1e1 - a * exp(-t / T)) * (sqrt(0.3e1) * pow(0.3141592654e1, -0.1e1 / 0.2e1) * x[2] *
                                 pow(pow(x[0], 0.2e1) + pow(x[1], 0.2e1) + pow(x[2], 0.2e1), -0.1e1 / 0.2e1) / 0.2e1 +
                                 0.1e1 / 0.2e1));
}

static RegisterScalarFunction regsca_test7_harmonic( "Test7Solution", Test7_chiSol);

double Test7_rhs3 (const DROPS::Point3DCL& p, double  t) {
    double a=0.8;
    double T=0.25;
    DROPS::Point3DCL x= (p - PosDrop - t*constant_wind(p, t));//--constant wind
    double inertia=a / T * exp(-t / T) * (sqrt(0.3e1) * pow(0.3141592654e1, -0.1e1 / 0.2e1) * x[2] *
                            pow(pow(x[0], 0.2e1) + pow(x[1], 0.2e1) + pow(x[2], 0.2e1), -0.1e1 / 0.2e1)
                                          / 0.2e1 + 0.1e1 / 0.2e1);
    double diffusion=-ParameterNS::sigma * ParameterNS::eps * (a * exp(-t / T) - 0.1e1) * sqrt(0.3e1) * x[2] * pow(pow(x[0], 0.2e1) + pow(x[1], 0.2e1) + pow(x[2], 0.2e1), -0.3e1 / 0.2e1) * pow(0.3141592654e1, -0.1e1 / 0.2e1);
    double well_potential=-ParameterNS::sigma / ParameterNS::eps * (a * exp(-t / T) - 0.1e1) * (sqrt(0.3e1) * x[2] +
                            sqrt(0.3141592654e1) * sqrt(pow(x[0], 0.2e1) + pow(x[1], 0.2e1) + pow(x[2], 0.2e1))) *
                          (exp(-t / T) * sqrt(0.3e1) * a * x[2] + exp(-t / T) * sqrt(0.3141592654e1) * sqrt(pow(x[0], 0.2e1) +
                                                                                                            pow(x[1], 0.2e1) +
                                                                                                            pow(x[2], 0.2e1)) * a -
                           sqrt(0.3e1) * x[2] + sqrt(0.3141592654e1) * sqrt(pow(x[0], 0.2e1) + pow(x[1], 0.2e1) + pow(x[2], 0.2e1)))
                          * (exp(-t / T) * sqrt(0.3e1) * a * x[2] + exp(-t / T) * sqrt(0.3141592654e1) * sqrt(pow(x[0], 0.2e1) + pow(x[1], 0.2e1) + pow(x[2], 0.2e1)) * a - sqrt(0.3e1) * x[2])
                          * pow(0.3141592654e1, -0.3e1 / 0.2e1) * pow(pow(x[0], 0.2e1) + pow(x[1], 0.2e1) + pow(x[2], 0.2e1), -0.3e1 / 0.2e1) / 0.8e1;
    return(inertia+diffusion+well_potential);
}

static RegisterScalarFunction regsca_rhs7_harmonic( "Test7RHS", Test7_rhs3);
...
////////////////////////////////////////////////////////Xianmin add this new example--
//The shape of the surface is more or less general (taken from Dziuk1988)
DROPS::Point3DCL general_wind (const DROPS::Point3DCL& p, double t)
{
	Point3DCL wd;
	wd[0]=0.1*p[0]*cos(t);
	wd[1]=0.2*p[1]*sin(t);
...
static RegisterVectorFunction regvec_general_wind( "GeneralWind", general_wind);

double general_lset (const Point3DCL& p, double t)
{
	 DROPS::Point3DCL x( p - P.get<DROPS::Point3DCL>("SurfSeparation.Exp.PosDrop"));
	 x[0]=x[0]*std::exp(-0.1*sin(t));
	 x[1]=x[1]*std::exp(-0.2+0.2*cos(t));
	 x[2]=x[2]*std::exp(-0.2*sin(t));
	 return   (x[0]-(x[2]*x[2]))*(x[0]-x[2]*x[2])+x[1]*x[1]+x[2]*x[2]-1;

}
static RegisterScalarFunction regsca_general_lset( "GeneralLset", general_lset);

DROPS::Point3DCL normal_general (const DROPS::Point3DCL& p, double t)
{
	 DROPS::Point3DCL x( p - P.get<DROPS::Point3DCL>("SurfSeparation.Exp.PosDrop"));
	x[0]=x[0]*std::exp(-0.1*sin(t));
	x[1]=x[1]*std::exp(-0.2+0.2*cos(t));
	x[2]=x[2]*std::exp(-0.2*sin(t));
	DROPS::Point3DCL normal_;//--general wind
	normal_[0]=2*(x[0]-x[2]*x[2])*std::exp(-0.1*sin(t));
	normal_[1]=2*x[1]*std::exp(-0.2+0.2*cos(t));
	normal_[2]=-4*(x[0]-x[2]*x[2])*x[2]*std::exp(-0.2*sin(t))+2*x[2]*std::exp(-0.2*sin(t));
	if(normal_.norm()>0.000001)
		return normal_/normal_.norm();
	else return normal_;
}
static RegisterVectorFunction regvec_normal_general( "NormalGeneral", normal_general);

double general_sol (const Point3DCL& p, double t)
{
    if (t==0)
...
static RegisterScalarFunction regsca_general_sol( "GeneralSol", general_sol);

DROPS::Point3DCL general_surfgradsol (const DROPS::Point3DCL& p, double t)
{
	Point3DCL surfgrad;
    return surfgrad;
}
static RegisterVectorFunction regvec_general_surfgradsol( "GeneralSurfGradSol", general_surfgradsol);


double general_rhs (const Point3DCL& p, double t)
{
 	return 0; //Ex3
}
static RegisterScalarFunction regsca_general_rhs( "GeneralRhs", general_rhs);

////////////////////////////////////////////////////////Xianmin add this new example--
DROPS::Point3DCL rotate_wind (const DROPS::Point3DCL& p, double )
{
	Point3DCL wd;
	wd[0]= -2.5*M_PI*p[1];
	wd[1]= 2.5*M_PI*p[0];
...
static RegisterVectorFunction regvec_rotate_wind( "RotateWind", rotate_wind);

double rotate_lset (const Point3DCL& p, double t)
{
	 DROPS::Point3DCL x(p);
	 x[0]= p[0]*std::cos(2.5*M_PI*t) + p[1]*std::sin(2.5*M_PI*t)-1.0/2;//-1.0/2;
	 x[1]= -p[0]*std::sin(2.5*M_PI*t) + p[1]*std::cos(2.5*M_PI*t);
	return   x[0]*x[0] +x[1]*x[1] +x[2]*x[2]-1;

}
static RegisterScalarFunction regsca_rotate_lset( "RotateLset", rotate_lset);

DROPS::Point3DCL normal_rotate (const DROPS::Point3DCL& p, double t)
{
	DROPS::Point3DCL center;
	center[0]=1.0/2*std::cos(2.5*M_PI*t);//0*
	center[1]=1.0/2*std::sin(2.5*M_PI*t);//0*
	center[2]=0;
	DROPS::Point3DCL x= (p - center);//--rotate wind
	if(x.norm()>=0.000001)
		return x/x.norm();
	else
		return x;
}
static RegisterVectorFunction regvec_normal_rotate( "NormalRotate", normal_rotate);

double rotate_sol (const Point3DCL& p, double t)
{
	 DROPS::Point3DCL x(p);
	 double epsilon=P.get<double>("SurfSeparation.Epsilon");
	 double r0=0.8;
	 double r=sqrt(1-(1-r0*r0)*exp(2*t));
	 double theta=std::asin(r);
	 x[0]= p[0]*std::cos(2.5*M_PI*t) + p[1]*std::sin(2.5*M_PI*t)-1.0/2;//-1.0/2;
	 double theta1=std::acos(x[0]/sqrt(x[0]*x[0]+x[1]*x[1]+x[2]*x[2]));
	 double d=(theta-theta1);
//	 x[1]= -p[0]*std::sin(0.2*M_PI*t) + p[1]*std::cos(0.2*M_PI*t);
	return  std::tanh(d/std::sqrt(2)/epsilon);//lv3: p[0]*p[1]*p[2]-0.0012020219;//lv2:p[0]*p[1]*p[2]-0.005175103;//
}
static RegisterScalarFunction regsca_rotate_sol( "RotateSol", rotate_sol);

DROPS::Point3DCL rotate_surfgradsol (const DROPS::Point3DCL& p, double t)
{
	Point3DCL surfgrad;
    return surfgrad;
}
static RegisterVectorFunction regvec_rotate_surfgradsol( "RotateSurfGradSol", rotate_surfgradsol);


double rotate_rhs (const Point3DCL&, double )
{
 	return 0; //Ex3
}
static RegisterScalarFunction regsca_rotate_rhs( "RotateRhs", rotate_rhs);
//////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////Xianmin add this new example--
DROPS::Point3DCL pulse_wind (const DROPS::Point3DCL& p, double t)
{
	Point3DCL wd;
	double delta=1.0/6;
	double n=16*M_PI;
	double vel=0.5*std::pow(std::sqrt(1+delta*cos(n*t)),-3)*n*delta*sin(n*t);
...
static RegisterVectorFunction regvec_pulse_wind( "PulseWind", pulse_wind);

double pulse_lset (const Point3DCL& p, double t)
{
	 DROPS::Point3DCL x(p);
	 double delta=1.0/6;
	double n=16*M_PI;
	double R=1/sqrt(1+delta*cos(n*t));
	return   x[0]*x[0] +x[1]*x[1] +x[2]*x[2]-R*R;

}
static RegisterScalarFunction regsca_pulse_lset( "PulseLset", pulse_lset);

DROPS::Point3DCL normal_pulse (const DROPS::Point3DCL& p, double t)
{
...
static RegisterVectorFunction regvec_normal_pulse( "NormalPulse", normal_pulse);

double pulse_sol (const Point3DCL& p, double t)
{
	 DROPS::Point3DCL x(p);
	 double epsilon=P.get<double>("SurfSeparation.Epsilon");
	 double r0=0.8;//!!!R0~=1
	// double r=sqrt(1-(1-r0*r0)*exp(2*t));
	 double theta=std::asin(r0);//!!!R0~=1
	 double theta1=std::acos(x[0]/sqrt(x[0]*x[0]+x[1]*x[1]+x[2]*x[2]));
	 double d=(theta-theta1);
//	 x[1]= -p[0]*std::sin(0.2*M_PI*t) + p[1]*std::cos(0.2*M_PI*t);
	return  std::tanh(d/std::sqrt(2)/epsilon);//lv3: p[0]*p[1]*p[2]-0.0012020219;//lv2:p[0]*p[1]*p[2]-0.005175103;//
}
static RegisterScalarFunction regsca_pulse_sol( "PulseSol", pulse_sol);

DROPS::Point3DCL pulse_surfgradsol (const DROPS::Point3DCL& p, double t)
{
	Point3DCL surfgrad;
    return surfgrad;
}
static RegisterVectorFunction regvec_pulse_surfgradsol( "PulseSurfGradSol", pulse_surfgradsol);


double pulse_rhs (const Point3DCL&, double )
{
 	return 0; //Ex3
}
static RegisterScalarFunction regsca_pulse_rhs( "PulseRhs", pulse_rhs);
//////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////
...
AllenCahnP1BaseCL* make_allencahn_timedisc( MultiGridCL& mg, LevelsetP2CL& lset,
                                              VecDescCL& v, const BndDataCL<Point3DCL>& Bnd_v,
                                              const ParamCL& P, const double & dist=0)
{
    AllenCahnP1BaseCL* ret= 0;
    const std::string method= P.get<std::string>( "SurfSeparation.Method");

    //std::valarray<double> id(1.,v.Data.size()/3);
    //MatrixCL Ident;
     if (method == std::string( "NarrowBandStabilization"))
        ret= new AllenCahnNarrowBandStblP1CL( mg,
                                     P.get<double>("SurfSeparation.Theta"),
                                     P.get<double>("SurfSeparation.Epsilon"),
                                     &v, Bnd_v, lset.Phi, lset.GetBndData(), the_normal_fun, dist,
                                     P.get<DROPS::Point3DCL>("Mesh.E1")[0]/(2.*P.get<double>("Mesh.N1")), 2,//S=beta_s,
                                     P.get<int>("SurfSeparation.Solver.Iter"), P.get<double>("SurfSeparation.Solver.Tol"),
                                     P.get<double>("SurfSeparation.XFEMReduced"));
    else
        throw DROPSErrCL( std::string( "make_allencahn_timedisc: Unknown method '") + method + std::string( "'.\n"));
...
//phase separation, Allen-Cahn
void Strategy1 (DROPS::MultiGridCL& mg, DROPS::AdapTriangCL& adap, DROPS::LevelsetP2CL& lset)
{
    using namespace DROPS;
    //set up output
    std::string dirname  = P.get<std::string>("VTK.VTKDir")  + "AllenCahn" + "_" + P.get<std::string>("SurfSeparation.Exp.Levelset")
    + "/"+ "Eps=" + std::to_string(float(P.get<double>("SurfSeparation.Epsilon")))
    +"h=" + std::to_string(float(P.get<DROPS::Point3DCL>("Mesh.E1")[0]/(2*P.get<double>("Mesh.N1")))) + "_dt="
    + std::to_string(float(P.get<double>("Time.FinalTime")/P.get<double>("Time.NumSteps")));
    ...
    if (P.get<std::string>("SurfSeparation.Exp.Levelset") == std::string( "AxisScalingLset"))
        dynamic_cast<DistMarkingStrategyCL*>( adap.get_marking_strategy())->SetDistFct( axis_scaling_lset_ini);
         ...
         dynamic_cast<DistMarkingStrategyCL*>( adap.get_marking_strategy())->SetDistFct( the_lset_fun);
    ...
    //Allen-Cahn equation
    std::unique_ptr<AllenCahnP1BaseCL> timediscp( make_allencahn_timedisc( mg, lset, v, Bnd_v, P, dist));
    AllenCahnP1BaseCL& timedisc= *timediscp;
    ...
    //Since there is no explicit solution, We do not compute the errors here
  /*  double L_2x_chi_err= L2_error( lset.Phi, lset.GetBndData(), timedisc.GetConcentr(), the_conc_sol_fun);
 ...
 double L_inftL_2x_chi_err= L_2x_chi_err;
    ...
    BndDataCL<> ifbnd( 0);
    double concentration=Integral_Gamma( mg, lset.Phi, lset.GetBndData(), make_P1Eval(  mg, ifbnd, timedisc.ic)) ;
    std::cout << "initial concentration on \\Gamma: " << concentration<< '\n';
    std::ofstream out1("concentration.txt");
    out1<<concentration<<std::endl;
    dynamic_cast<DistMarkingStrategyCL*>( adap.get_marking_strategy())->SetDistFct( lset);
    for (int step= 1; step <= P.get<int>("Time.NumSteps"); ++step) {
        std::cout << "======================================================== step " << step << ":\n";
        ScopeTimerCL timer( "Strategy: Time-loop");
        ...
        timedisc.DoStep0( cur_time);
        concentration=Integral_Gamma( mg, lset.Phi, lset.GetBndData(), make_P1Eval(  mg, ifbnd, timedisc.ic));
        std::cout << "concentration on \\Gamma: " << concentration << '\n';
        out1<<concentration<<std::endl;
    //Since there is no explicit solution, We do not compute the errors here
  /*
        L_2x_chi_err= L2_error( lset.Phi, lset.GetBndData(), timedisc.GetConcentr(), the_conc_sol_fun);
...
//         }
    ...
    std::cout << std::endl;
 /*   std::cout << "L_2 energy error norm of 3- variable:\t"  << std::sqrt(L_2tH_1x_chi_err_sq) << std::endl;
    log_global << "L_2 energy error norm of 3- -variable:\t" << std::sqrt(L_2tH_1x_chi_err_sq) << std::endl;
    log_global << "C error norm of 3- -variable:\t" << L_inftL_2x_chi_err << "\t" <<  std::endl;*/
    //delete &lset2;
    log_global.close();
}
...
int  main (int argc, char* argv[])
{
  try {
    ...
    std::string dirname  = P.get<std::string>("VTK.VTKDir")  + "AllenCahn" + "_" + P.get<std::string>("SurfSeparation.Exp.Levelset")  + "/"  "h=" + std::to_string(float(P.get<DROPS::Point3DCL>("Mesh.E1")[0]/(2*P.get<double>("Mesh.N1")))) + "_dt=" + std::to_string(float(P.get<double>("Time.FinalTime")/P.get<double>("Time.NumSteps")));
...
//        }
   ...
   //     Strategy( mg, adap, lset);//Cahn-Hilliard
    	  Strategy1( mg, adap, lset);//Allen-Cahn
